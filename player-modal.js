...
(function () {
  ...
  function nameFromLink(a) {
    try { return new URL(a.href, location.href).searchParams.get("name"); }
    catch (e) { return null; }
  }

  // resolved below depending on context
  let openModal = (name) =>
    (location.href = "player.html?name=" + encodeURIComponent(name) + "&embed=1");
  ...
  // one delegated handler catches every <a> link to a player card
  document.addEventListener("click", (e) => {
    const a = e.target.closest('a[href*="player.html?name="]');
  ...
  openModal = function (name) {
    ...
    frame.src = "player.html?name=" + enc + "&embed=1";
    full.href = "player.html?name=" + enc;
