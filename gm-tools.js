...
(function (global) {
  ...
  function recommendMoves(B, team, opts) {
    ...
    // rate rides the bundle (re-fit nightly); the fallback literal is pinned
    // by a suite test to the CURRENT fit so it can never go stale again
    // (this line shipped $5.25M/WAR for two weeks after the fit moved)
    var DPW = B.dollarsPerWar || 4710846, FLOOR = B.leagueMin || 775000;
